...
function addSignatureReturns(f) {
  if (navOptions.methodHeadingReturns) {
    ...
    f.signature = '<span class="signature">' + ('') + '</span>' + '<span class="type-signature">' + (returnTypes.length ? ' &rarr; {' + returnTypes.join('|') + '}' : '') + '</span>';
  }
  else {
...
exports.publish = function(taffyData, opts, tutorials) {
  ...
  if (conf.sort || conf.sort === undefined) {
    var sortOrder = typeof conf.sort === 'string' ? opts.sort : 'longname, version, since';
    data.sort(sortOrder);
  }
